...
function tryUnlockVideo() {
  ...
  video.play()
    .then(() => video.pause())   // kita tetap kontrol manual
    .catch(() => {});
}

// gesture yang diizinkan browser
window.addEventListener("click", tryUnlockVideo, { once:true });
window.addEventListener("touchstart", tryUnlockVideo, { once:true });
window.addEventListener("scroll", tryUnlockVideo, { once:true });
